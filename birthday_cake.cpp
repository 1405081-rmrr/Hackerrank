...
int birthdayCakeCandles(vector<int> candles) {
    ...
    {
        if(candles[i]>=_max) _max=candles[i];
    }
    cout<<_max<<endl;
    int _count=0;
    for(int i=0;i<candles.size();i++)
    {
        if(_max==candles[i]) _count++;
    }
    cout<<_count<<endl;
    return _count;
...
int main()
{
    ...
    int candles_count = atoi(ltrim(rtrim(candles_count_temp)).c_str());
    ...
    for (int i = 0; i < candles_count; i++) {
        int candles_item = atoi(candles_temp[i].c_str());
